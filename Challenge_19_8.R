...
for (i in 1:6){
  ...
  traintest[inValid_12, names(traintest) %in% c("u", "v", "ws", "wd")] <- NA;
  traintest[inValid_24, names(traintest) %in% c("u12", "v12", "ws12", "wd12")] <- NA;
  traintest[inValid_36, names(traintest) %in% c("u24", "v24", "ws24", "wd24")] <- NA;
